...
@section('content')
    <form class="max-w-sm mx-auto" id="createProductForm" method="POST" action="{{ route('product.store') }}">
        ...
        <div class="mb-1">
            <label for="name"
                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Product Name') }} <span
        ...
        <div class="mb-1">
            <label for="description"
                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Description') }} <span
                    class="text-red-500">*</span></label>
        ...
        <div class="mb-1">
            <label for="image_path"
                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Image Path') }} <span
                    class="text-red-500">*</span></label>
...
<span class="text-red-500
    ...
    <div class="mb-1">
        <label for="select_category"
            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Select Category') }} <span
                class="text-red-500">*</span></label>
        @foreach ($categories as $index => $category)
            <input id="select_category_{{ $index }}" name="select_category[]" type="checkbox"
                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                value="{{ $category->id }}" @if (in_array($category->id, old('select_category', []))) checked @endif>
            <label for="select_category_{{ $index }}"
                class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300 pr-5">{{ $category->name }}</label><br>
        @endforeach
    ...
    <button type="submit"
        ...
        id="addproduct">{{ __('Add Product') }}</button>
</form>
@endsection
